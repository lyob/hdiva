...
from c_training.configs.diva_config import (
...
def main():
    ...
    wandb_callback = WandbArtifactCallback(
        every_n_epochs=config.checkpoint_every_n_epochs, config=config
    )
    ...
    if config.resume_from_checkpoint:
        if config.use_pretrained_denoiser_only:
            ...
            model = DiVA_Lightning(
                config=config, pretrained_wandb_config=pretrained_wandb_config
            )
        else:
            ...
            model = DiVA_Lightning.load_from_checkpoint(
                pretrained_checkpoint_dir, config=config
            )
    else:
    ...
    rename_checkpoint_folder(
        ...
        checkpoint_dir=os.path.join(
            base_dir, f"c_training/lightning_checkpoints/{config.model_name}"
        ),
    )
