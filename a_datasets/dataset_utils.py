...
from a_datasets.custom_dataset_classes import CustomDataset, DiskDataset
...
def load_dataset_from_hf(
    ...
    if dataset_size != 0:
        ...
        train_dataset, test_dataset = torch.utils.data.random_split(
            transformed_dataset, [train_size, test_size], generator=torch.Generator().manual_seed(42)
        )
    else:
...
def prepare_dataset(dataset_name, model_name, img_dim, dataset_size, local_rank):
    if dataset_name == "celeba" or dataset_name == "celeba_color" or dataset_name == "shapes3d":
        ...
        train_dataset, _ = load_dataset_from_hf(
            dataset_type=dataset_name, with_label=False, img_dim=img_dim, dataset_size=dataset_size
        )
...
def move_celeba_data_to_tmp(dataset_resolution: int):
    ...
    sysrsync.run(
        source=f"datasets/celeba/attribute_images_{dataset_resolution}x{dataset_resolution}.pt",
        destination=f"/tmp/attribute_images_{dataset_resolution}x{dataset_resolution}.pt",
    )


def load_hdisk_dataset(config, load_dataloader: bool = False):
    if config.dataset_name == "hdisks3":
        from a_datasets.hdisks.hdisks3 import random_two_disk_dataset

        data = random_two_disk_dataset(
            img_size=config.input_dim, outer_radius=4, transition_width=2, d=10, num_imgs=config.dataset_size
        )[0]
    elif config.dataset_name == "hdisks3_stochastic":
        from a_datasets.hdisks.hdisks3_stochastic import random_two_disk_dataset
