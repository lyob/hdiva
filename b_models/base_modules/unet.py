...
class FirstBlock(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.conv = nn.Conv2d(
            args.num_channels + 1, args.num_kernels, args.kernel_size, padding=args.padding, bias=args.bias
        )
...
class DownBlock(nn.Module):
    def __init__(self, args, b, l, norm="bn"):
        super().__init__()
        self.in_channels = args.num_kernels * (2 ** (b - 1)) if l == 0 else args.num_kernels * (2**b)
        self.out_channels = args.num_kernels * (2**b)
        self.conv = nn.Conv2d(
            self.in_channels, self.out_channels, args.kernel_size, padding=args.padding, bias=args.bias
        )
        if norm == "bn":
            self.norm = nn.BatchNorm2d(self.out_channels) if args.bias else BF_batchNorm(self.out_channels)
        elif norm == "gn":
            self.norm = (
                nn.GroupNorm(32, self.out_channels) if args.bias else nn.GroupNorm(32, self.out_channels, affine=False)
            )
        else:
...
class MiddleBlock(nn.Module):
    def __init__(self, args, l, norm="bn"):
        super().__init__()
        b = args.num_blocks - 1
...
class FinalBlock(nn.Module):
    def __init__(self, args):
        super().__init__()
        self.conv = nn.Conv2d(
            args.num_kernels, args.num_channels, kernel_size=args.kernel_size, padding=args.padding, bias=False
        )
...
class UNet(nn.Module):
    def __init__(self, args):
        ...
        for l in range(args.num_mid_conv):
            self.mid.append(MiddleBlock(args, l, norm=args.norm))
    ...
    def init_encoder_block(self, b, args):
        if b == 0:
            for l in range(1, args.num_enc_conv):
                self.down.append(DownBlock(args, b, l, norm=args.norm))
        else:
    ...
    def init_decoder_block(self, b, args):
        if b == 0:
            for l in range(args.num_dec_conv - 1):
                self.up.append(UpBlock(args, b, l, norm=args.norm))
        else:
