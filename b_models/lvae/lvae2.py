...
"""Ladder Variational Autoencoder (LVAE) with two latent layers. Differs from lvae1.py 
in that it outputs the log std from each block.
"""
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.decoder):
            if i == 0:
                ...
                # layer_kl = self.compute_kl_with_samples(z_top, top_posterior_params, prior_params=None)
                ...
                # layer_kl = self.compute_kl_with_samples(z_merged, posterior_params, td_prior_params)
