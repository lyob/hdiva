...
####################
    ...
    def forward(self, clean_x, t_y_max=None, unconditional=False, debug=False):
        ...
        mu_y_from_x, logvar_y_from_x, mu_z_from_x, logvar_z_from_x = self.recnet.x_to_y_to_z(clean_x)
        ...
        rate_z = self.compute_z_rate(mu_z_from_xg, logvar_z_from_xg, noisy_x, z_sample_up_from_x)
        ...
        rate_y = self.compute_y_rate(score_z_given_ye, score_x_given_ye, score_z_given_ye_and_x)
        ...
        pred_y_noise_guided_by_x_and_z = self.predict_noise_using_guidance_score(noisy_y, t_tensor_y, pred_y_unguided, score_x_and_z_given_ye)
    ...
    def conditional_generation(self, N:int, target_x: torch.Tensor):
        ...
        self.noisy_y = torch.randn((N, self.recnet.input_dim)).to(target_x.device).requires_grad_(True)
        
        for t in range(self.num_timesteps_y - 1, -1, -1):
            self.reverse_one_timestep_y(self.noisy_y, t, mu_y_from_x, logvar_y_from_x, z_sample_up_from_x)

        y_sample_down_from_x_and_z = self.noisy_y

        # x denoiser with y guidance
        self.noisy_x = torch.randn((N, self.recnet.input_dim)).to(target_x.device).requires_grad_(True)

        for t in range(self.num_timesteps_x - 1, -1, -1):
            self.reverse_one_timestep_x(self.noisy_x, t, y_sample_down_from_x_and_z)
    ...
    @torch.no_grad()
    def unconditional_sample(self, N, denoiser_type:Literal["x", "y"] = "x"):
        ...
        if denoiser_type == "x":
            ...
            dim = self.recnet.input_dim
        elif denoiser_type == "y":
            denoiser = self.denoiser_y
            dim = self.recnet.latent_dim_y

        # x denoiser only
        noisy_signal = torch.randn((N, dim), device=device)

        for t in range(self.num_timesteps_x - 1, -1, -1):
            noisy_signal = self.reverse_one_timestep_unconditional(noisy_signal, denoiser, t, device)
    ...
    def stage2_generation(self, N:int, target_input:torch.Tensor):
        ...
        # x denoiser with y guidance
        noisy_x = torch.randn((N, self.recnet.input_dim)).to(device).requires_grad_(True)

        for t in range(self.num_timesteps_x - 1, -1, -1):
            noisy_x = self.reverse_one_timestep_x(noisy_x, t, y_sample_up_from_x)
