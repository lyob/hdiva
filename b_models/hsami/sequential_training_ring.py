...
from a_datasets.ring.data import WrappedPhaseRing
from a_datasets.ring.data_flat import WrappedPhaseRingFlat
from b_models.configs.hsami_config import HSAMI_Ring_Training_Config
...
@dataclass
class SequentialTrainingConfig(HSAMI_Ring_Training_Config):
    # Which stages to run (e.g. [1, 2] or [2] to resume from a prior run)
    stages: list = field(default_factory=lambda: [1, 2, 3, 4])
    # Run name to resume from when the first stage > 1 (e.g. "sequential_20260618_015523")
    resume_from_run: str = "sequential_20260620_133934"
    ...
    stage4_den_x_lr: float = 2e-5
    stage4_enc_xy_lr: float = 2e-5
    stage4_den_y_lr: float = 2e-5
    stage4_enc_yz_lr: float = 2e-5
    stage4_beta_y: float = 1e-3
    stage4_beta_z: float = 1e-2
    stage4_t_y_curriculum_steps: int = 120000
...
def save_sequential_config(config, run_dir):
    parent_field_names = {f.name for f in dataclasses.fields(HSAMI_Ring_Training_Config)}
...
def train():
    ...
    if config.dataset_name == "ring_flat":
        dataset = WrappedPhaseRingFlat(
            n=config.n,
            m=config.m,
            kappa=config.kappa,
            r0=config.r0,
    ...
    checkpoint_dir = os.path.join(project_root, "c_training", "hsami_checkpoints_ring")
